...
interface CoursesClientProps {
  /** Server-fetched first page — seeds the grid so SSR HTML carries real course
   *  content (titles + links) instead of an empty skeleton. */
...
export default function CoursesClient({
  ...
  // Until the client fetch resolves (and after SSR), fall back to the server data
  // so there is never an empty flash and crawlers always see real courses.
  const displayCourses = courses.length > 0 ? courses : initialCourses
  const displayTotal   = total || initialTotal
  const displayPages   = pages || initialPages
  const hasMore        = filters.page < displayPages
    ...
    <div className="min-h-screen w-full min-w-0 max-w-full overflow-x-clip bg-base-100">
      <div className="mx-auto max-w-7xl px-3 py-6 sm:px-6 sm:py-8 md:py-12 lg:px-8">
          ...
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
            ...
            {displayTotal > 0 && (
              <div className="flex-shrink-0 self-start px-3 py-1.5 sm:self-auto rounded-xl border border-primary/20 bg-primary/10">
                <span className="text-xs font-bold text-primary">{displayTotal} {t('courses.count')}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <CourseGrid
          ...
          loading={loading && displayCourses.length === 0}
          emptyText={
            debouncedSearch
        ...
        {!loading && hasMore && (
          ...
          <p className="text-center text-xs text-base-content/25 mt-8">
            {t('courses.allShown').replace('{total}', displayTotal.toString()).includes('{total}')
              ? t('courses.allShown').replace('{total}', displayTotal.toString())
              : `${t('filter.all')} ${displayTotal} ${t('courses.allShown')}`}
          </p>
      </div>
    </div>
  )
